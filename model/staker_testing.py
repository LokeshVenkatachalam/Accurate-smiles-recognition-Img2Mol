...
STAKER_IMAGE_PATH = ABSOLUTE_PATH + 'data/Staker'
STAKER_CDDD_PATH = ABSOLUTE_PATH + 'data/Staker_embeddings.csv'
...
image_list = []
for i in indices:
    image_list.append(os.path.join(STAKER_IMAGE_PATH,"{}.png".format(i)))
staker_images = torch.cat([torch.unsqueeze(load_image(image), 0) for image in image_list], dim=0)

staker_cddd_df = pd.read_csv(STAKER_CDDD_PATH, delimiter=',',  header=None)
staker_cddd = torch.from_numpy(staker_cddd_df.values).float().to(device)
staker_cddd = staker_cddd[:opt['dataset_size']]

b_dataset = []
for i in range(len(staker_images)):
    b_dataset.append([staker_images[i], staker_cddd[i]])
...
for i, (batch_x, test_labels) in enumerate(benchmarkloader, 0):
    ...
    file1 = open("staker_benchmark_predictions.pkl", "wb")
    np.save(file1, pred_np, allow_pickle=False)
    file1.close()
    file2 = open("staker_benchmark_test_labels.pkl", "wb")
